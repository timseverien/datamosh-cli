...
module.exports = async (source, destination, options = {}) => {
    ...
    const quality = Number.parseFloat(options.quality);
    ...
    if (quality) {
        ffmpegArguments.push(...[
            '-crf', Math.trunc(mathf.mix(51, 0, quality)).toString(),
            '-q:a', Math.trunc(mathf.mix(31, 1, quality)).toString(),
            '-q:v', Math.trunc(mathf.mix(31, 1, quality)).toString(),
        ]);
    }
